 ...
 */
int interpret_input(char *input, uint32_t *arg1, uint32_t *arg2, int *operator) {
  ...
  // Note: BE CAREFUL with strtok().. when writing threaded code, you must use
  // the reentrant version, strtok_r()
 ...
 */
uint32_t do_operation(uint32_t arg1, uint32_t arg2, int operator) {
  switch (operator)
  {
    case ADD:
      return arg1 + arg2;
